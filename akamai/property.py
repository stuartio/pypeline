...
class Property(Akamai):
    ...
    def listActivations(self, propertyId):
        path = '/papi/v1/properties/{propertyId}/activations'.format(propertyId = propertyId, version = version)
        result = self.get(path = path)
    ...
    def updateVersion(self, propertyId, propertyVersion, rules, ruleFormat):
        ...
        headers = {}
        if ruleFormat is not None:
            headers['Content-Type'] = 'application/vnd.akamai.papirules.{ruleFormat}+json'.format(ruleFormat = ruleFormat)
