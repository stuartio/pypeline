...
def get_config(folder):
    ...
    if os.path.exists(CONFIG_FILE):
        ...
        click.secho('pipeline.json does not exist at "{f}". Please confirm location or use --name parameter to specify another folder'.format(f = folder), fg='red')
        sys.exit(1)
...
def update(ctx, environment_name, notes):
    ...
    try:
        rules_update_result = PROPERTY_CLIENT.updateVersion(property['propertyId'], update_version, rules, CONFIG['ruleFormat'])
    except:
    ...
    click.secho('Setting hostnames for version {v} of property {p}'.format(v = update_version, p = property['propertyName']), fg='yellow')
    try:
...
def activate(ctx, environment_name, network, email):
    ...
    if len(pending_activations) > 0:
        click.echo('There is a pending activation of version {v} of property {p}. Please wait until this is complete then update and try again'.format(v = pending_activations[0]['propertyVersion'], p = pending_activations[0]['propertyName'], n = network))
        sys.exit(1)
...
def create(ctx, name, rule_format):
    ...
    if os.path.exists(pipeline_folder):
        click.secho('Folder "{f}" already exists. Please delete this folder or choose another'.format(f = pipeline_folder), fg='red')
        sys.exit(1)
    else:
...
@click.option('--JSONPaths', 'variable_paths', required=False, help="Comma-separated list of JSONPaths to locate value in rule tree. Only required if using positional variables")
...
@cli.command('status')
@click.option('--environment', 'environment_name', required=False, help="Specific environment to check. If omitted all environments will be displayed")
@click.pass_context
